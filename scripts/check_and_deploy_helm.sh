...
if kubectl get namespace ${CLUSTER_NAMESPACE}; then
  ...
  # Altering this to fail if the namespace is not found
  echo -e "FAILED: Namespace ${CLUSTER_NAMESPACE} not found."
  exit 1
fi
...
helm upgrade ${RELEASE_NAME} ${CHART_PATH} ${HELM_TLS_OPTION} --install --debug --dry-run --set image.repository=${IMAGE_REPOSITORY},image.tag=${IMAGE_TAG},image.pullSecret=${IMAGE_PULL_SECRET_NAME},ingress.hosts={${APP_HOSTNAME}},ingress.tls.hosts={${APP_HOSTNAME}} ${HELM_UPGRADE_EXTRA_ARGS} --namespace ${CLUSTER_NAMESPACE}

echo -e "Deploying into: ${PIPELINE_KUBERNETES_CLUSTER_NAME}/${CLUSTER_NAMESPACE}."
helm upgrade ${RELEASE_NAME} ${CHART_PATH} ${HELM_TLS_OPTION} --install --set image.repository=${IMAGE_REPOSITORY},image.tag=${IMAGE_TAG},image.pullSecret=${IMAGE_PULL_SECRET_NAME},ingress.hosts={${APP_HOSTNAME}},ingress.tls.hosts={${APP_HOSTNAME}} ${HELM_UPGRADE_EXTRA_ARGS} --namespace ${CLUSTER_NAMESPACE}
...
#DEPLOYMENT_NAME=$( helm get all ${HELM_TLS_OPTION} ${RELEASE_NAME} --namespace ${CLUSTER_NAMESPACE} | yq read -d'*' --tojson - | jq -r | jq -r --arg image "$IMAGE_REPOSITORY:$IMAGE_TAG" ' select (.kind=="Deployment") | . as $adeployment  | $adeployment.metadata.name' )
DEPLOYMENT_NAME=$( helm get all ${HELM_TLS_OPTION} ${RELEASE_NAME} --namespace ${CLUSTER_NAMESPACE} | yq read -d'*' --tojson - | jq -r  ' .[] | select (.kind=="Deployment") | .metadata.name' )
...
echo "=========================================================="
helm get manifest ${HELM_TLS_OPTION} ${RELEASE_NAME} --namespace ${CLUSTER_NAMESPACE} | yq read -d'*' --tojson - | jq -r  '.[] | select (.kind=="Deployment") | .metadata.labels.app '
...
APP_NAME=$( helm get manifest ${HELM_TLS_OPTION} ${RELEASE_NAME} --namespace ${CLUSTER_NAMESPACE} | yq read -d'*' --tojson - | jq -r  '.[] | select (.kind=="Deployment") | .metadata.labels.app' )
echo -e "APP: ${APP_NAME}"
echo "DEPLOYED PODS:"
